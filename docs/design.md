...
Phase 3a부터 **사용자에게 표시되는 UI 텍스트는 한국어**로 작성한다 — 워드마크(스마트 블로그), 페이지 제목(저장된 글 / 새 글 쓰기 / 글 편집 / 설정), 버튼(저장 / 발행 / 편집 / 취소 / 삭제 / 생성하기 / 새 글 쓰기), 폼 라벨(제목 / 요약 / 본문 / 저장소 / 브랜치 / 최근 커밋), 토스트 메시지 전부. status 표시(`draft` → "초안")도 한국어로 매핑하되, **published 상태의 카드 라벨만 예외로 "PUBLISHED" 타자기 결 영문**을 쓴다 (Phase 3b — 폰트 결과 시스템 도장 인상을 살리기 위함). 내부 enum값(`draft` / `published`)은 영문 그대로 두고 PostCard 등 표시 지점에서 매핑한다.
...
:root {
  ...
  /* Typewriter — 시스템 도장·고정 라벨 (Phase 3b 추가, status 라벨 전용) */
  --font-typewriter: "Special Elite", "Courier Prime", "Nanum Gothic Coding", monospace;
}
```
...
| **손글씨 cursive** | `--font-cursive` | 사람의 즉흥적 표시 | 편집기 placeholder, 빈 상태 메시지, AI draft 옆 짧은 첨언, toast 알림 |
| **타자기 라벨 typewriter** | `--font-typewriter` | 시스템 도장·고정 라벨 | status PUBLISHED 라벨 (Phase 3b부터 — 이전엔 cursive 자리) |
...
내부 enum `draft` / `published`를 PostCard `STATUS_DISPLAY`에서 표시 라벨로 매핑한다. **Phase 3b부터 published 표시는 타자기 결**의 "PUBLISHED" 라벨로 변경 (이전 Phase 3a의 cursive 도장 결 + rotate `발행됨`에서 교체). draft는 현재 카드에 노출되지 않는다.

```
- 위치: position absolute, top var(--space-3) / right var(--space-3) — 카드 우상단 (PinnedSurface가 nearest positioned ancestor 역할)
- font-family: var(--font-typewriter) — 'Special Elite', 'Courier Prime', monospace
- font-size: var(--text-meta-size) (11px)
- letter-spacing: 0.25em
- color: var(--color-accent-wax)
- text-transform: uppercase
- opacity: 0.85
- transform 없음, border 없음 (Phase 3b 결 — rotate 도장 결 폐기)
```

타자기 라벨은 영어 "PUBLISHED"로 표시 — 폰트의 결과 letter-spacing이 영문 활자체에서 정확히 의도된 인상이 나오고, status 라벨이 한국어 본문과 시각적으로 다른 화자(시스템 도장)임을 분명히 한다. 내부 enum (`published`)도 영어이므로 dev native와 결이 맞다.
...
### 5.10 Read view — side-by-side layout (Phase 4)

`/post/:id` 페이지 전용 레이아웃. 발행된 글 본문과 원본 commit 변경을 좌-우로 나란히 보여준다.

```
- 페이지 컨테이너: max-inline-size var(--size-read-page-max) (1100px), margin-inline auto
- 좌-우 grid: 3fr 2fr, gap var(--space-6)
- 좁은 화면 @media (max-width: 800px) → 1열로 stack (글 위, diff 아래)
```

**좌측 (글) — PinnedSurface variant="letter"**
- 페이퍼 클립 좌상단 + ruled-letter 배경 (편집기와 동일한 결, 단 인풋 X)
- 메타: 브랜치 태그 + 커밋 날짜 (mono small caps)
- 제목: italic serif `--text-h1-size`
- 요약: italic serif `--text-preview-size`
- 본문: react-markdown 렌더, `--text-body-size`/`--line-ruled-letter` (편집기 프리뷰와 동일)
- **PUBLISHED 라벨은 표시 X** — read view에 있다는 사실이 곧 발행 상태이므로 redundant

**우측 (원본 변경) — PinnedSurface variant="card"**
- letter보다 가벼운 결로 prose가 주연임을 시각적으로 분명히 함 (memo의 ruled 배경은 patch monospace와 시각 충돌해서 card로 결정).
- 헤더: "원본 변경" — mono small caps, `--color-accent-brass` + `border-block-end: 0.5px solid var(--color-accent-brass)`
- 파일별 섹션 (sequence):
  - filename: mono `font-weight: 700`, `--text-meta-size`, sepia
  - patch: `<pre>` 안에 line span 시리즈, `white-space: pre`, `overflow-x: auto`
- 파일 사이: `--border-double-ink-thin` border-top + space-4 gap
- empty: `commitFiles` 빈 배열 또는 undefined → cursive "원본 변경 정보가 없습니다" 단일 메시지

**Patch line 색 (semantic tokens)**
- `+` 시작 라인: `background var(--color-add-bg)` (옅은 cream-green `#E5EFE0`), `color var(--color-add-text)` (sepia ink-100)
- `-` 시작 라인: `background var(--color-remove-bg)` (옅은 cream-wine `#F4E2DE`), `color var(--color-remove-text)` (sepia ink-100)
- `@` 시작 라인 (hunk header): `color var(--color-text-muted)`, 배경 없음
- 그 외 context 라인: paper 톤 그대로

**Actions (페이지 하단)**
- `SecondaryButton "목록으로"` → `/saved`
- `SecondaryButton "편집"` → `/post/:id/edit`
...
- v0.5 (2026-05-24, Phase 4): Read view (`/post/:id`) — letter (글) + card (원본 변경) 3:2 side-by-side, 800px 아래로는 stack. diff add/remove semantic 토큰 (`--color-add-bg` / `--color-remove-bg`, paper-cream + 옅은 tint). Post entity에 `commitFiles?` persist. PostCard published 제목이 read view link.
